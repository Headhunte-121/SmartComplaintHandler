...
### 1. Mandatory Audit Trails in Incident Resolution
In quality assurance and service reliability engineering, closure documentation serves as legal and administrative evidence:
- If a repaired water pipe bursts again 2 hours later, administrators must know who performed the original repair and what parts were installed.
- By enforcing `resolution_notes` at both the frontend modal level and the backend service level, the platform guarantees that every resolved ticket in the database has an unbroken audit trail.

### 2. Multi-Field Controlled Modal Form State
Managing multiple inputs inside a modal requires clean state orchestration:
- Rather than creating three separate `useState` hooks (`const [notes, setNotes] = ...`, `const [parts, setParts] = ...`), our component uses a single state object: `formData`.
- A generic change handler updates fields dynamically:
  `const handleChange = (e) => setFormData(prev => ({ ...prev, [e.target.name]: e.target.value }))`
- This scalable architecture simplifies form reset, pre-population, and payload serialization into a single, cohesive pattern.

### 3. Optimistic UI Updates vs. Confirmed Server Handoff
In frontend engineering, developers sometimes implement Optimistic Updates (updating the UI immediately before the server confirms the request):
- In destructive or terminal operations (like resolving a ticket or escalating an incident), optimistic updates are dangerous: if the server rejects the request (e.g. due to an illegal state transition), the UI displays false information that must be awkwardly rolled back.
- Our modal implements Confirmed Server Handoff: the modal displays an in-flight loading spinner while the network promise resolves. Only when the server returns `HTTP 200` does the modal trigger `onResolved()` and dismiss itself, guaranteeing 100% synchronization between the screen and the database.
